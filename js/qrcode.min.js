/**
 * EasyQRCode / QRCode generator for offline LAN Wi-Fi sharing
 * Generates an SVG or Canvas QR code cleanly in vanilla JS
 */
(function(window) {
  // Minimal standalone QR Code Generator (Reed-Solomon + Byte mode)
  // Standard implementation compatible with all modern browsers
  ...
  QRCodeModel.prototype = {
    ...
    setupPositionAdjustPattern: function() {
      var pos = [6, 18, 22, 26, 30, 34][this.typeNumber - 1];
      if (!pos) return;
      for (var i = 0; i < 1; i++) {
        var row = pos;
        var col = pos;
    ...
    setupTypeInfo: function(test, maskPattern) {
      ...
      var bits = data; // simplified type info
      for (var i = 0; i < 15; i++) {
  ...
  // Helper QRCode rendering to container element
  window.QRCode = function(targetEl, options) {
    ...
    this.options = {
      ...
      correctLevel: (typeof options.correctLevel === "number") ? options.correctLevel : 0
    };
  ...
  window.QRCode.prototype.makeCode = function(text) {
    var model = new QRCodeModel(0, this.options.correctLevel);
    model.addData(text);
    model.make();
    ...
    if (typeof this.targetEl.replaceChildren === "function") {
      this.targetEl.replaceChildren(canvas);
    } else {
      while (this.targetEl.firstChild) {
        this.targetEl.removeChild(this.targetEl.firstChild);
      }
      this.targetEl.appendChild(canvas);
    }
  };
